...
- `debug` ➡️ `(eventKey, eventData) => {}` _... for debugging errors, need to enable it via `ManagerOptions#advancedOptions#enableDebugEvents: true`_
...
- `resumed` ➡️ `(node, payload, players) => {}`
    - Resuming needs to be handled manually by you _(aka add the players to the manager)_
...
function updateNextTrackGapLess(guildId: string) { // this function is for nodelink users relevant for gapless audio.
