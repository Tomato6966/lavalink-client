...
- � Advanced Queue Filtering & Searching
    - Filter tracks by title, author, duration, source, and more
    - Find specific tracks with flexible search criteria
    - Sort queue by duration, title, or author
    - Get track ranges for custom implementations

- �😍 Included Filter & Equalizer Management

- 👍 Multiple Player Options _for easier use_
    - onDisconnect -> Player Destroy / auto Reconnect _on when its queue got empty_
