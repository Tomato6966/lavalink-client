...
Hot-Tipp: You can use player variables in order to enable/disable this system during runtime, e.g. by `player.setData("autoplay_disabled", true)` and check it in the autoplayFunction by `const isAutoPlayDisabled = player.getData("autoplay_disabled") === true;`
