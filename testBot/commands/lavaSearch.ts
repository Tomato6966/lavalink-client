...
export default {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((o) =>
            o.setName("query").setDescription("What to play?").setAutocomplete(true).setRequired(true),
        ),
    execute: async (client, interaction) => {
        ...
        if (!fromAutoComplete[Number(query.replace("autocomplete_", ""))]?.pluginInfo?.url)
            return interaction.reply({ flags: [MessageFlags.Ephemeral], content: "Nothing found" });
        ...
        const response = (await player.search(
            { query: fromAutoComplete[Number(query.replace("autocomplete_", ""))].pluginInfo.url },
            interaction.user,
        ...
        await interaction.respond(
            res[`${type}s`]
                .map((t: LavaSearchFilteredResponse, i) => ({
                    name: `[${t.pluginInfo.totalTracks || NaN} Tracks] ${t.info.title || t.info.name} (by ${t.pluginInfo.author || t.info?.author || "Unknown-Author"})`.substring(
                        0,
                        100,
                    ),
