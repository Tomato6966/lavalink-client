...
export default {
    ...
    autocomplete: async (client, interaction) => {
        ...
        const res = await player.lavaSearch(
            { query: focussedQuery, types: [type], source: interaction.options.getString("source") as LavaSrcSearchPlatformBase },
            interaction.user,
        );
        ...
        await interaction.respond(
            res[`${type}s`]
                .map((t: LavaSearchFilteredResponse, i) => ({
                    name: `[${t.pluginInfo.totalTracks || NaN} Tracks] ${t.info.title || t.info.name} (by ${t.pluginInfo.author || "Unknown-Author"})`.substring(
                        0,
                        100,
                    ),
