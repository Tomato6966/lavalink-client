import { type CommandInteractionOptionResolver, type GuildMember, SlashCommandBuilder } from 'discord.js';
...
export default {
	data: new SlashCommandBuilder()
		.setName('seek')
		.setDescription('Seek the position within the current Track')
		.addIntegerOption(o =>
			o.setName('position').setDescription('To what position (seconds) to seek to?').setRequired(true),
		),
	execute: async (client, interaction) => {
		...
		const position =
			((interaction.options as CommandInteractionOptionResolver).getInteger('position') as number) * 1000;
		if (position > player.queue.current.info.duration || position < 0)
		...
		await interaction.reply({
			ephemeral: true,
			content: `Seeked to: \`${formatMS_HHMMSS(player.position)}\``,
		});
	},
} as Command;
