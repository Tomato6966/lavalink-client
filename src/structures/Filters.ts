...
export class FilterManager {
	...
	checkFiltersState(oldFilterTimescale?: Partial<TimescaleFilter>) {
		...
		this.filters.lavalinkLavaDspxPlugin.normalization =
			Object.values(this.data.pluginFilters.normalization || {}).length > 0;
	...
	lavalinkFilterPlugin = {
		...
		toggleReverb: async (delays = [0.037, 0.042, 0.048, 0.053], gains = [0.84, 0.83, 0.82, 0.81]): Promise<boolean> => {
			...
			this.data.pluginFilters['lavalink-filter-plugin'].reverb.delays = this.filters.lavalinkFilterPlugin.reverb
				? []
				: delays;
			this.data.pluginFilters['lavalink-filter-plugin'].reverb.gains = this.filters.lavalinkFilterPlugin.reverb
				? []
				: gains;
