...
export class Player {
  ...
  public get position() {
    return (
      this.lastPosition +
      (this.lastPositionChange ? Date.now() - this.lastPositionChange : 0)
    );
  }
  ...
  constructor(
    ...
    if (!this.node || typeof this.node.request !== "function") {
      if (
        typeof this.options.node === "string" &&
        this.LavalinkManager.options?.advancedOptions?.enableDebugEvents
      ) {
        this.LavalinkManager.emit(
          "debug",
          DebugEvents.PlayerCreateNodeNotFound,
          {
            state: "warn",
      ...
      this.node =
        least.filter((v) =>
          options.vcRegion
            ? v.options?.regions?.includes(options.vcRegion)
            : true
        )[0] ||
        least[0] ||
        null;
   ...
   */
  public clearData() {
    const toKeep = Object.keys(this.data).filter((v) =>
      v.startsWith("internal_")
    );
  ...
  public getAllData(): Record<string, unknown> {
    return Object.fromEntries(
      Object.entries(this.data).filter((v) => !v[0].startsWith("internal_"))
    );
  }
  ...
  async play(options: Partial<PlayOptions> = {}) {
    if (this.get("internal_queueempty")) {
      if (this.LavalinkManager.options?.advancedOptions?.enableDebugEvents) {
        this.LavalinkManager.emit(
          "debug",
          DebugEvents.PlayerPlayQueueEmptyTimeoutClear,
          {
            state: "log",
    ...
    if (
      options?.clientTrack &&
      (this.LavalinkManager.utils.isTrack(options?.clientTrack) ||
        this.LavalinkManager.utils.isUnresolvedTrack(options.clientTrack))
    ) {
      if (this.LavalinkManager.utils.isUnresolvedTrack(options.clientTrack)) {
        try {
          ...
          if (
            this.LavalinkManager.options?.autoSkipOnResolveError === true &&
            this.queue.tracks[0]
          )
      ...
      if (
        (typeof options.track?.userData === "object" ||
          typeof options.clientTrack?.userData === "object") &&
        options.clientTrack
      )
    ...
    if (options?.track?.encoded || options?.track?.identifier) {
      ...
      if (this.LavalinkManager.options?.advancedOptions?.enableDebugEvents) {
        this.LavalinkManager.emit(
          "debug",
          DebugEvents.PlayerPlayWithTrackReplace,
          {
            state: "log",
    ...
    if (
      this.queue.current &&
      this.LavalinkManager.utils.isUnresolvedTrack(this.queue.current)
    ) {
      if (this.LavalinkManager.options?.advancedOptions?.enableDebugEvents) {
        this.LavalinkManager.emit(
          "debug",
          DebugEvents.PlayerPlayUnresolvedTrack,
          {
            state: "log",
    ...
    if (
      (typeof finalOptions.position !== "undefined" &&
        isNaN(finalOptions.position)) ||
      (typeof finalOptions.position === "number" &&
        (finalOptions.position < 0 ||
          finalOptions.position >= this.queue.current.info.duration))
    )
      throw new Error(
        "PlayerOption#position must be a positive number, less than track's duration"
      );
    if (
      (typeof finalOptions.volume !== "undefined" &&
        isNaN(finalOptions.volume)) ||
      (typeof finalOptions.volume === "number" && finalOptions.volume < 0)
    )
      throw new Error("PlayerOption#volume must be a positive number");
    if (
      (typeof finalOptions.endTime !== "undefined" &&
        isNaN(finalOptions.endTime)) ||
      (typeof finalOptions.endTime === "number" &&
        (finalOptions.endTime < 0 ||
          finalOptions.endTime >= this.queue.current.info.duration))
    )
      throw new Error(
        "PlayerOption#endTime must be a positive number, less than track's duration"
      );
    if (
      typeof finalOptions.position === "number" &&
      typeof finalOptions.endTime === "number" &&
      finalOptions.endTime < finalOptions.position
    )
      throw new Error(
        "PlayerOption#endTime must be bigger than PlayerOption#position"
      );
  ...
  async search(
    ...
    if (
      ...
      !this.node.info.sourceManagers.includes("bandcamp")
    ) {
      if (this.LavalinkManager.options?.advancedOptions?.enableDebugEvents) {
        this.LavalinkManager.emit(
          "debug",
          DebugEvents.BandcampSearchLokalEngine,
          {
            state: "log",
  ...
  async seek(position: number) {
    ...
    if (position < 0 || position > this.queue.current.info.duration)
      position = Math.max(
        Math.min(position, this.queue.current.info.duration),
        0
      );
  ...
  async setRepeatMode(repeatMode: RepeatMode) {
    if (!["off", "track", "queue"].includes(repeatMode))
      throw new RangeError(
        "Repeatmode must be either 'off', 'track', or 'queue'"
      );
  ...
  async skip(skipTo: number = 0, throwError: boolean = true) {
    if (
      ...
      (throwError || (typeof skipTo === "boolean" && skipTo === true))
    )
  ...
  public async connect() {
    if (!this.options.voiceChannelId)
      throw new RangeError(
        "No Voice Channel id has been set. (player.options.voiceChannelId)"
      );
  ...
  public async changeVoiceState(data: {
  ...
  public async destroy(
    reason?: DestroyReasons | string,
    disconnect: boolean = true
  ) {
    //  [disconnect -> queue destroy -> cache delete -> lavalink destroy -> event emit]
    if (
      this.LavalinkManager.options.advancedOptions?.debugOptions.playerDestroy
        .debugLog
    )
      console.log(
        `Lavalink-Client-Debug | PlayerDestroy [::] destroy Function, [guildId ${
          this.guildId
        }] - Destroy-Reason: ${String(reason)}`
      );
    ...
    if (this.get("internal_destroystatus") === true) {
      if (this.LavalinkManager.options?.advancedOptions?.enableDebugEvents) {
        this.LavalinkManager.emit(
          "debug",
          DebugEvents.PlayerDestroyingSomewhereElse,
          {
            state: "warn",
  ...
  public async changeNode(
    newNode: LavalinkNode | string,
    checkSources: boolean = true
  ) {
    const updateNode =
      typeof newNode === "string"
        ? this.LavalinkManager.nodeManager.nodes.get(newNode)
        : newNode;
    ...
    if (checkSources) {
      const isDefaultSource = (): boolean => {
        // check if defaultSearchPlatform is enabled on newNode
        try {
      ...
      if (this.queue.current || this.queue.tracks.length) {
        ...
        const trackSources = new Set(
          [this.queue.current, ...this.queue.tracks].map(
            (track) => track.info.sourceName
          )
        );
        ...
        if (missingSources.length)
          throw new RangeError(
            `Sources missing for Node ${updateNode.id}: ${missingSources.join(
              ", "
            )}`
          );
      }
    ...
    try {
      await this.connect();
      const hasSponsorBlock = this.node.info?.plugins?.find(
        (v) => v.name === "sponsorblock-plugin"
      );
      if (hasSponsorBlock) {
        const sponsorBlockCategories = this.get(
          "internal_sponsorBlockCategories"
        );
        if (
          Array.isArray(sponsorBlockCategories) &&
          sponsorBlockCategories.length
        ) {
  ...
  public async moveNode(node?: string) {
    try {
      if (!node)
        node = Array.from(
          this.LavalinkManager.nodeManager.leastUsedNodes("playingPlayers")
