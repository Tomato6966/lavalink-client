...
export class Player {
	...
	constructor(options: PlayerOptions, LavalinkManager: LavalinkManager) {
		...
		if (!this.node || typeof this.node.request !== 'function') {
			...
			this.node =
				least.filter(v => (options.vcRegion ? v.options?.regions?.includes(options.vcRegion) : true))[0] ||
				least[0] ||
				null;
		}
		if (!this.node)
	...
	async play(options: Partial<PlayOptions> = {}) {
		if (this.get('internal_queueempty')) {
			if (typeof this.options.node === 'string' && this.LavalinkManager.options?.advancedOptions?.enableDebugEvents) {
				this.LavalinkManager.emit('debug', DebugEvents.PlayerPlayQueueEmptyTimeoutClear, {
					state: 'log',
					message:
						'Player was called to play something, while there was a queueEmpty Timeout set, clearing the timeout.',
		...
		if (
			...
			if (
				(typeof options.track?.userData === 'object' || typeof options.clientTrack?.userData === 'object') &&
				options.clientTrack
			)
		...
		if (options?.track?.encoded || options?.track?.identifier) {
			...
			if (typeof this.options.node === 'string' && this.LavalinkManager.options?.advancedOptions?.enableDebugEvents) {
				this.LavalinkManager.emit('debug', DebugEvents.PlayerPlayWithTrackReplace, {
					state: 'log',
					message:
						'Player was called to play something, with a specific track provided. Replacing the current Track and resolving the track on trackStart Event.',
					functionLayer: 'Player > play()',
				});
			}
		...
		if (
			...
			(typeof finalOptions.position === 'number' &&
				(finalOptions.position < 0 || finalOptions.position >= this.queue.current.info.duration))
		)
		...
		if (
			...
			(typeof finalOptions.endTime === 'number' &&
				(finalOptions.endTime < 0 || finalOptions.endTime >= this.queue.current.info.duration))
		)
		...
		if (['bcsearch', 'bandcamp'].includes(Query.source) && !this.node.info.sourceManagers.includes('bandcamp')) {
			if (typeof this.options.node === 'string' && this.LavalinkManager.options?.advancedOptions?.enableDebugEvents) {
				this.LavalinkManager.emit('debug', DebugEvents.BandcampSearchLokalEngine, {
					state: 'log',
					message:
						'Player.search was called with a Bandcamp Query, but no bandcamp search was enabled on lavalink, searching with the custom Search Engine.',
	 ...
	 */
	public async destroy(reason?: DestroyReasons | string, disconnect = true) {
