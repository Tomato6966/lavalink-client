...
export class LavalinkNode {
    ...
    public getHealthStatus(thresholds?: HealthStatusThresholdOptions): HealthStatusObject {
        ...
        // Check if overloaded
        const isOverloaded =
            metrics.cpuLoad > cpuThresholds.fair ||
            metrics.memoryUsage > memoryThresholds.fair ||
            metrics.frameDeficit > 100;
        const isCritical =
            metrics.cpuLoad > cpuThresholds.poor ||
            metrics.memoryUsage > memoryThresholds.poor ||
            metrics.frameDeficit > 500;
        // Determine status
        ...
        let penaltyScore =
            metrics.players + // Player count penalty (each player adds base penalty)
    ...
    private executeReconnect() {
        ...
        // Clean up old entries if array grows too large
            ...
            case "playerUpdate":
                {
                    ...
                    if (!player)
                        return this._emitDebugEvent(DebugEvents.PlayerUpdateNoPlayer, {
                            state: "error",
