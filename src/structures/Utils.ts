...
export class ManagerUtils {
	...
	isNodeOptions(data: LavalinkNodeOptions) {
		...
		if (
			'regions' in data &&
			(!Array.isArray(data.regions) || (!data.regions.every(v => typeof v === 'string') && data.regions !== undefined))
		)
			return false;
	...
	validateQueryString(node: LavalinkNode, queryString: string, sourceString?: LavalinkSearchPlatform): void {
		...
		if (this.LavalinkManager.options?.linksAllowed === false)
		...
		if (
			(SourceLinksRegexes.YoutubeMusicRegex.test(queryString) || SourceLinksRegexes.YoutubeRegex.test(queryString)) &&
		...
		if (
			...
			throw new Error(
				"Query / Link Provided for this Source but Lavalink Node has not 'http' enabled, which is required to have 'deezer' to work",
			);
		}
	...
	validateSourceString(node: LavalinkNode, sourceString: SearchPlatform) {
		...
		if (
			source === 'speak' &&
			!node.info?.plugins?.find(c => c.name.toLowerCase().includes(LavalinkPlugins.DuncteBot_Plugin.toLowerCase()))
		) {
			throw new Error("Lavalink Node has not 'speak' enabled, which is required to have 'speak' work");
		}
		if (
			source === 'tts' &&
			!node.info?.plugins?.find(c => c.name.toLowerCase().includes(LavalinkPlugins.GoogleCloudTTS.toLowerCase()))
		) {
...
async function applyUnresolvedData(resTrack: Track, data: UnresolvedTrack, utils: ManagerUtils) {
	...
	if (utils?.LavalinkManager?.options?.playerOptions?.useUnresolvedData === true) {
		...
		if (
			(resTrack.info.title === 'Unknown title' || resTrack.info.title === 'Unspecified description') &&
 ...
 *
 * @param {UnresolvedTrack} data UnresolvedTrack with information about the Track
 * @param {Player} player The Player to use for searching the Track
 * @returns {Promise<Track|undefined>} The closest Track found or undefined if nothing was found
 * @throws {RangeError} If the player doesn't have a lavalink node
 * @throws {SyntaxError} If the track uri / title / encoded Base64 string is required for unresolved tracks
 * @throws {SyntaxError} If The requester is required
 * @throws {RangeError} If Track is not an unresolved Track
 */
async function getClosestTrack(data: UnresolvedTrack, player: Player): Promise<Track | undefined> {
	if (!player?.node) throw new RangeError('No player with a lavalink node was provided');
	...
	return await player
		.search(
			{
				...
				source:
					sourceName !== 'twitch' && sourceName !== 'flowery-tts'
						? sourceName
						: player.LavalinkManager.options?.playerOptions?.defaultSearchPlatform,
			},
			data.requester,
		)
		.then((res: SearchResult) => {
			...
			if (data.info.duration && !trackToUse)
				trackToUse = res.tracks.find(
					track =>
						track.info?.duration >= data.info?.duration - 1500 && track?.info.duration <= data.info?.duration + 1500,
				);
