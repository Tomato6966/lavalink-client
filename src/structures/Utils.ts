...
export function parseLavalinkConnUrl(connectionUrl: string) {
  if (!connectionUrl.startsWith("lavalink://"))
    throw new Error(
      `ConnectionUrl (${connectionUrl}) must start with 'lavalink://'`
    );
...
export class ManagerUtils {
  ...
  buildTrack(data: LavalinkTrack | Track, requester: unknown) {
    ...
    try {
      ...
      if (
        ...
        typeof data?.userData?.requester === "object" &&
        data.userData.requester !== null
      ) {
        transformedRequester = this.getTransformedRequester(
          data.userData.requester
        );
      }
      ...
      const r = {
        ...
        pluginInfo: this.buildPluginInfo(
          data,
          "clientData" in data ? data.clientData : {}
        ),
        requester:
          transformedRequester ||
          this.getTransformedRequester(this.LavalinkManager?.options?.client),
      } as Track;
      Object.defineProperty(r, TrackSymbol, {
        configurable: true,
        value: true,
      });
      return r;
  ...
  buildUnresolvedTrack(
    query: UnresolvedQuery | UnresolvedTrack,
    requester: unknown
  ) {
    ...
    const unresolvedTrack: UnresolvedTrack = {
      ...
      async resolve(player: Player) {
        ...
        // assign new symbol
        Object.defineProperty(this, TrackSymbol, {
          configurable: true,
          value: true,
        });
  ...
  getTransformedRequester(requester: unknown) {
    try {
      return typeof this.LavalinkManager?.options?.playerOptions
        ?.requesterTransformer === "function"
        ? this.LavalinkManager?.options?.playerOptions?.requesterTransformer(
            requester
          )
        : requester;
    } catch (e) {
      if (this.LavalinkManager?.options?.advancedOptions?.enableDebugEvents) {
        this.LavalinkManager?.emit(
          "debug",
          DebugEvents.TransformRequesterFunctionFailed,
          {
            error: e,
            functionLayer: "ManagerUtils > getTransformedRequester()",
            message:
              "Your custom transformRequesterFunction failed to execute, please check your function for errors.",
            state: "error",
          }
        );
      }
  ...
  isNodeOptions(data: LavalinkNodeOptions) {
    ...
    if (
      ...
      (!Array.isArray(data.regions) ||
        (!data.regions.every((v) => typeof v === "string") &&
          data.regions !== undefined))
    )
      return false;
    if (
      "poolOptions" in data &&
      typeof data.poolOptions !== "object" &&
      data.poolOptions !== undefined
    )
      return false;
    if (
      "retryAmount" in data &&
      (typeof data.retryAmount !== "number" ||
        isNaN(data.retryAmount) ||
        (data.retryAmount <= 0 && data.retryAmount !== undefined))
    )
      return false;
    if (
      "retryDelay" in data &&
      (typeof data.retryDelay !== "number" ||
        isNaN(data.retryDelay) ||
        (data.retryDelay <= 0 && data.retryDelay !== undefined))
    )
      return false;
    if (
      "requestTimeout" in data &&
      (typeof data.requestTimeout !== "number" ||
        isNaN(data.requestTimeout) ||
        (data.requestTimeout <= 0 && data.requestTimeout !== undefined))
    )
      return false;
    return true;
  }
  ...
  isUnresolvedTrack(data: UnresolvedTrack | Track): data is UnresolvedTrack {
    ...
    return (
      typeof data === "object" &&
      (("info" in data && typeof data.info.title === "string") ||
        typeof data.encoded === "string") &&
      "resolve" in data &&
    ...
    try {
      ...
      if (this.LavalinkManager?.options?.advancedOptions?.enableDebugEvents) {
        this.LavalinkManager?.emit("debug", DebugEvents.GetClosestTrackFailed, {
          ...
          message:
            "Failed to resolve track because the getClosestTrack function failed.",
          state: "error",
        });
      }
      throw e;
  ...
  validateQueryString(
    ...
    if (!queryString.trim().length)
      throw new Error(
        `Query string is empty, please provide a valid query string.`
      );
    ...
    if (this.LavalinkManager.options?.linksBlacklist?.length > 0) {
      if (this.LavalinkManager.options?.advancedOptions?.enableDebugEvents) {
        this.LavalinkManager.emit(
          "debug",
          DebugEvents.ValidatingBlacklistLinks,
          {
            state: "log",
            message: `Validating Query against LavalinkManager.options.linksBlacklist, query: "${queryString}"`,
            functionLayer:
              "(LavalinkNode > node | player) > search() > validateQueryString()",
          }
        );
      }
      if (
        this.LavalinkManager.options?.linksBlacklist.some(
          (v) =>
            (typeof v === "string" &&
              (queryString.toLowerCase().includes(v.toLowerCase()) ||
                v.toLowerCase().includes(queryString.toLowerCase()))) ||
            (isRegExp(v) && v.test(queryString))
        )
      ) {
        throw new Error(
          `Query string contains a link / word which is blacklisted.`
        );
      }
    }
    ...
    if (this.LavalinkManager.options?.linksWhitelist?.length > 0) {
      if (this.LavalinkManager.options?.advancedOptions?.enableDebugEvents) {
        this.LavalinkManager.emit(
          "debug",
          DebugEvents.ValidatingWhitelistLinks,
          {
            state: "log",
      ...
      if (
        !this.LavalinkManager.options?.linksWhitelist.some(
          (v) =>
            (typeof v === "string" &&
              (queryString.toLowerCase().includes(v.toLowerCase()) ||
                v.toLowerCase().includes(queryString.toLowerCase()))) ||
        ...
        throw new Error(
          `Query string contains a link / word which isn't whitelisted.`
        );
      }
    }
    ...
    if (
      ...
      throw new Error(
        "Query / Link Provided for this Source but Lavalink Node has not 'youtube' enabled"
      );
    }
    if (
      (SourceLinksRegexes.SoundCloudMobileRegex.test(queryString) ||
        SourceLinksRegexes.SoundCloudRegex.test(queryString)) &&
      !node.info?.sourceManagers?.includes("soundcloud")
    ) {
      throw new Error(
        "Query / Link Provided for this Source but Lavalink Node has not 'soundcloud' enabled"
      );
    }
    if (
      SourceLinksRegexes.bandcamp.test(queryString) &&
    ...
    if (
      SourceLinksRegexes.TwitchTv.test(queryString) &&
      !node.info?.sourceManagers?.includes("twitch")
    ) {
      throw new Error(
        "Query / Link Provided for this Source but Lavalink Node has not 'twitch' enabled"
      );
    }
    if (
      SourceLinksRegexes.vimeo.test(queryString) &&
      !node.info?.sourceManagers?.includes("vimeo")
    ) {
      throw new Error(
        "Query / Link Provided for this Source but Lavalink Node has not 'vimeo' enabled"
      );
    }
    if (
      SourceLinksRegexes.tiktok.test(queryString) &&
      !node.info?.sourceManagers?.includes("tiktok")
    ) {
      throw new Error(
        "Query / Link Provided for this Source but Lavalink Node has not 'tiktok' enabled"
      );
    }
    if (
      SourceLinksRegexes.mixcloud.test(queryString) &&
      !node.info?.sourceManagers?.includes("mixcloud")
    ) {
      throw new Error(
        "Query / Link Provided for this Source but Lavalink Node has not 'mixcloud' enabled"
      );
    }
    if (
      SourceLinksRegexes.AllSpotifyRegex.test(queryString) &&
      !node.info?.sourceManagers?.includes("spotify")
    ) {
      throw new Error(
        "Query / Link Provided for this Source but Lavalink Node has not 'spotify' enabled"
      );
    }
    if (
      SourceLinksRegexes.appleMusic.test(queryString) &&
      !node.info?.sourceManagers?.includes("applemusic")
    ) {
      throw new Error(
        "Query / Link Provided for this Source but Lavalink Node has not 'applemusic' enabled"
      );
    }
    if (
      SourceLinksRegexes.AllDeezerRegex.test(queryString) &&
      !node.info?.sourceManagers?.includes("deezer")
    ) {
      throw new Error(
        "Query / Link Provided for this Source but Lavalink Node has not 'deezer' enabled"
      );
    }
    if (
      SourceLinksRegexes.musicYandex.test(queryString) &&
      !node.info?.sourceManagers?.includes("yandexmusic")
    ) {
      throw new Error(
        "Query / Link Provided for this Source but Lavalink Node has not 'yandexmusic' enabled"
      );
    }
    if (
      SourceLinksRegexes.jiosaavn.test(queryString) &&
      !node.info?.sourceManagers?.includes("jiosaavn")
    ) {
      throw new Error(
    ...
    if (
      SourceLinksRegexes.tidal.test(queryString) &&
      !node.info?.sourceManagers?.includes("tidal")
    ) {
      throw new Error(
  ...
  transformQuery(query: SearchQuery) {
    ...
    const Query = {
      ...
      extraQueryUrlParams:
        typeof query !== "string" ? query.extraQueryUrlParams : undefined,
      source:
        sourceOfQuery ??
        this.LavalinkManager?.options?.playerOptions?.defaultSearchPlatform?.toLowerCase?.(),
    };
    const foundSource = Object.keys(DefaultSources)
      .find((source) =>
        Query.query?.toLowerCase?.()?.startsWith(`${source}:`.toLowerCase())
      )
      ?.trim?.()
    ...
    if (
      foundSource &&
      !["https", "http"].includes(foundSource) &&
      DefaultSources[foundSource]
    ) {
      Query.source = DefaultSources[foundSource]; // set the source to ytsearch:
      Query.query = Query.query.slice(
        `${foundSource}:`.length,
        Query.query.length
      ); // remove ytsearch: from the query
    }
    return Query;
  }
  ...
  validateSourceString(node: LavalinkNode, sourceString: SearchPlatform) {
    ...
    const source = DefaultSources[
      sourceString.toLowerCase().trim()
    ] as LavalinkSearchPlatform;
    if (!source)
      throw new Error(
        `Lavalink Node SearchQuerySource: '${sourceString}' is not available`
      );

    if (!node.info)
      throw new Error(
        "Lavalink Node does not have any info cached yet, not ready yet!"
      );
    ...
    if (
      ...
      !node.info?.plugins?.find((c) =>
        c.name
          .toLowerCase()
          .includes(LavalinkPlugins.DuncteBot_Plugin.toLowerCase())
      )
    ) {
      throw new Error(
    ...
    if (
      ...
      throw new Error(
        "Lavalink Node has not 'tidal' enabled, which is required to have 'tdsearch' work"
      );
    }
    if (source === "tdrec" && !node.info?.sourceManagers?.includes("tidal")) {
      throw new Error(
        "Lavalink Node has not 'tidal' enabled, which is required to have 'tdrec' work"
      );
    }
    if (
      source === "tts" &&
      !node.info?.plugins?.find((c) =>
        c.name
          .toLowerCase()
          .includes(LavalinkPlugins.GoogleCloudTTS.toLowerCase())
      )
    ) {
      throw new Error(
    ...
    if (
      ...
      throw new Error(
        "Lavalink Node has not 'flowery-tts' enabled, which is required to have 'ftts' work"
      );
    }
    if (
      source === "ymsearch" &&
    ...
    if (
      source === "vksearch" &&
      !node.info?.sourceManagers?.includes("vkmusic")
    ) {
      throw new Error(
        "Lavalink Node has not 'vkmusic' enabled, which is required to have 'vksearch' work"
      );
    }
    if (source === "vkrec" && !node.info?.sourceManagers?.includes("vkmusic")) {
      throw new Error(
        "Lavalink Node has not 'vkmusic' enabled, which is required to have 'vkrec' work"
      );
    }
    if (
      source === "qbsearch" &&
      !node.info?.sourceManagers?.includes("qobuz")
    ) {
      throw new Error(
        "Lavalink Node has not 'qobuz' enabled, which is required to have 'qbsearch' work"
      );
    }
    if (source === "qbisrc" && !node.info?.sourceManagers?.includes("qobuz")) {
      throw new Error(
        "Lavalink Node has not 'qobuz' enabled, which is required to have 'qbisrc' work"
      );
    }
    if (source === "qbrec" && !node.info?.sourceManagers?.includes("qobuz")) {
      throw new Error(
        "Lavalink Node has not 'qobuz' enabled, which is required to have 'qbrec' work"
      );
    }
    if (
      source === "gaanasearch" &&
...
export class MiniMap<K, V> extends Map<K, V> {
  ...
  public filter<K2 extends K>(
    fn: (value: V, key: K, miniMap: this) => key is K2
  ): MiniMap<K2, V>;
  ...
  public map<T>(
    fn: (value: V, key: K, miniMap: this) => T,
    thisArg?: unknown
  ): T[] {
    if (typeof thisArg !== "undefined") fn = fn.bind(thisArg);
...
export async function queueTrackEnd(
  player: Player,
  dontShiftQueue: boolean = false
) {
  if (
    player.queue.current &&
    !player.queue.current?.pluginInfo?.clientData?.previousTrack
  ) {
  ...
  const nextSong = dontShiftQueue
    ? null
    : (player.queue.tracks.shift() as Track);

  try {
    if (nextSong && player.LavalinkManager.utils.isUnresolvedTrack(nextSong))
      await (nextSong as UnresolvedTrack).resolve(player);
    ...
    if (player.LavalinkManager.options?.advancedOptions?.enableDebugEvents) {
      player.LavalinkManager.emit(
        "debug",
        DebugEvents.PlayerPlayUnresolvedTrackFailed,
        {
          state: "error",
          error: error,
    ...
    if (
      !dontShiftQueue &&
      player.LavalinkManager.options?.autoSkipOnResolveError === true &&
...
async function applyUnresolvedData(
  resTrack: Track,
  data: UnresolvedTrack,
  utils: ManagerUtils
) {
  ...
  if (
    utils?.LavalinkManager?.options?.playerOptions?.useUnresolvedData === true
  ) {
    // overwrite values
    ...
    if (
      (resTrack.info.title === "Unknown title" ||
        resTrack.info.title === "Unspecified description") &&
  ...
  for (const key of Object.keys(data.info))
    if (
      typeof resTrack.info[key] === "undefined" &&
      key !== "resolve" &&
      data.info[key]
    )
...
async function getClosestTrack(
  ...
  if (!data?.info?.title && typeof data.encoded !== "string" && !data.info.uri)
    throw new SyntaxError(
      "the track uri / title / encoded Base64 string is required for unresolved tracks"
    );
  ...
  if (typeof data.encoded === "string") {
    const r = await player.node.decode.singleTrack(
      data.encoded,
      data.requester
    );
  ...
  if (typeof data.info.uri === "string") {
    const r = await player
      .search({ query: data?.info?.uri }, data.requester)
      .then((v) => v.tracks?.[0] as Track | undefined);
    if (r) return applyUnresolvedData(r, data, player.LavalinkManager.utils);
  }
  // search the track as closely as possible
  const query = [data.info?.title, data.info?.author]
    .filter((str) => !!str)
    .join(" by ");
  ...
  return await player
    .search(
      {
        query,
        source:
          sourceName !== "twitch" && sourceName !== "flowery-tts"
            ? sourceName
            : player.LavalinkManager.options?.playerOptions
    ...
    .then((res: SearchResult) => {
      ...
      if (data.info.author && !trackToUse)
        trackToUse = res.tracks.find(
          (track) =>
            ...
            new RegExp(`^${escapeRegExp(data.info?.title)}$`, "i").test(
              track.info?.title
            )
        );
      ...
      if (data.info.duration && !trackToUse)
        trackToUse = res.tracks.find(
          (track) =>
            track.info?.duration >= data.info?.duration - 1500 &&
            track?.info.duration <= data.info?.duration + 1500
        );
      // try to find via isrc
      if (data.info.isrc && !trackToUse)
        trackToUse = res.tracks.find(
          (track) => track.info?.isrc === data.info?.isrc
        );
      // apply unresolved data and return the track
      return applyUnresolvedData(
        trackToUse || res.tracks[0],
        data,
        player.LavalinkManager.utils
      );
    });
}
